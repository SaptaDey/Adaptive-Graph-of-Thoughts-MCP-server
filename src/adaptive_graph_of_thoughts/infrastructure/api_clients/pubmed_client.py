...
class PubMedClient:
    ...
    def __init__(self, settings: LegacyConfig):
        ...
        logger.info(
            f"PubMedClient initialized with max_results: {self.config.max_results}"
        )
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb):
        await self.http_client.client.__aexit__(
    ...
    def _construct_common_params(self) -> dict[str, Any]:
        ...
        if (
            hasattr(self.config, "email") and self.config.email
    ...
    def _parse_esummary_response(self, xml_text: str) -> list[PubMedArticle]:
        ...
        try:
            ...
            for doc_sum in root.findall("DocSum"):
                ...
                for item in doc_sum.findall("Item"):
                    ...
                    if name == "Title" and item.text:
                        ...
                        authors = [
                            author.text
                            for author in item.findall("Item[@Name='Author']")
                            if author.text
                        ]
                        ...
                        name == "PubDate" and item.text
                    ):  # Publication date (e.g., "2023" or "2023 May")
                ...
                if "title" not in data:  # Skip if essential info like title is missing
                    logger.warning(
                        f"Skipping article PMID {pmid} due to missing title in eSummary."
                    )
                    continue
            ...
            logger.error(
                f"Unexpected error parsing PubMed eSummary for PMID {current_pmid_parsing}: {e}"
            )
    ...
    async def search_articles(
        ...
        try:
            ...
            if (
                "esearchresult" in esearch_data
                and "idlist" in esearch_data["esearchresult"]
            ):
                pmids = esearch_data["esearchresult"]["idlist"]
                logger.debug(f"Found {len(pmids)} PMIDs from eSearch: {pmids}")
            else:
                logger.warning(
                    f"No PMIDs found or unexpected eSearch response format for query: {query}"
                )
                return []
            ...
            logger.error(
                f"Failed to decode JSON from PubMed eSearch for query '{query}': {e}"
            )
        ...
        try:
            response_esummary = await self.http_client.get(
                "esummary.fcgi", params=esummary_params
            )
            ...
            for article in articles:
                if article.pmid:  # Ensure pmid is valid before fetching abstract
                    try:
                        ...
                        logger.warning(
                            f"Could not fetch abstract for PMID {article.pmid}: {e}"
                        )
                        # Decide if this should halt processing or just log and continue
                        article.abstract = None

            logger.info(
                f"Successfully retrieved and parsed {len(articles)} articles from PubMed for query: '{query}'."
            )
            return articles
        except (APIHTTPError, APIRequestError) as e:
            logger.error(
                f"PubMed eSummary API error for PMIDs '{','.join(pmids)}': {e}"
            )
    ...
    async def fetch_abstract(self, pmid: str) -> Optional[str]:
        ...
        try:
            response_efetch = await self.http_client.get(
                "efetch.fcgi", params=efetch_params
            )
            ...
            for abstract_node in root.findall(".//AbstractText"):  # Common path
                ...
                if text_content:  # Ensure there's actual text
                    ...
                    nlm_category = abstract_node.get(
                        "NlmCategory"
                    ...
                    if label:
                        ...
                        nlm_category
                    ):  # Use NlmCategory as a fallback label if Label is not present
            ...
            if abstract_texts:
                ...
                for other_abstract_node in root.findall(
                ...
                if abstract_texts:
                    full_abstract = "\n".join(abstract_texts)
                    logger.debug(
                        f"Successfully fetched abstract (from OtherAbstract) for PMID: {pmid}"
                    )
                    return full_abstract
            ...
            raise PubMedClientError(
                f"XML parsing error for eFetch (PMID {pmid}): {e}"
            ) from e
        except (APIHTTPError, APIRequestError) as e:
            ...
            raise PubMedClientError(
                f"Unexpected error in fetch_abstract (PMID {pmid}): {e}"
            ) from e
...
async def main_pubmed_test():
    ...
    try:
        ...
        if not settings.pubmed:
            logger.warning(
                "PubMed config not set in settings.yaml or env vars; using mock for testing."
            )
            settings.pubmed = PubMedConfig(api_key=None, max_results=20)
    except Exception as e:
        logger.warning(
            f"Could not load global settings ({e}), using mock PubMed config for testing."
        )
        # Manually create a Settings object with a default PubMedConfig
        settings = Settings(pubmed=PubMedConfig(api_key=None, max_results=20))
    ...
    async with PubMedClient(settings=settings) as client:
        try:
            ...
            if articles:
                ...
                for article in articles:
                    logger.info(
                        f"  PMID: {article.pmid}, Title: {article.title}, DOI: {article.doi}"
                    )
                    # Abstract is now fetched within search_articles, so it should be populated
                    if article.abstract:
                        logger.info(
                            f"    Abstract (first 150 chars): {article.abstract[:150]}..."
                        )
                    else:
                        logger.info(
                            f"    No abstract found or fetched for PMID {article.pmid}."
                        )
            else:
            ...
            logger.error(
                f"An unexpected error occurred during PubMed client test: {e}",
                exc_info=True,
            )
