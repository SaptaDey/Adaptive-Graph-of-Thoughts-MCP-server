...
def test_invalid_parameter_type(monkeypatch):
    """
    Tests that providing an invalid parameter type to the query method raises ValueError before any HTTP request is made.
    """
    monkeypatch.setenv("EXA_API_KEY", "dummy_key")
    client = ExaSearchClient()
...
def test_http_error_propagates(monkeypatch):
    """
    Tests that HTTP errors with status codes other than 401 raised by the client propagate as HTTPError exceptions.
    """
    ...
    def _raise_500(*_a, **_kw):
        ...
        This function is typically used in tests to mock server-side failures.
        """
