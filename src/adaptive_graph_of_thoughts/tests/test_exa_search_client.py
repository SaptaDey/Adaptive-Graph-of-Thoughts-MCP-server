...
def test_invalid_parameter_type(monkeypatch):
    ...
    with patch.object(ExaSearchClient, "_send_request") as mock_send:
        ...
        def test_unexpected_http_error(monkeypatch, exa_client):
        def _raise_500(*_a, **_kw):
        err = HTTPError("500 Server Error: Internal")
        err.response = MagicMock(status_code=500)
        raise err
        with patch.object(ExaSearchClient, "_send_request", side_effect=_raise_500):
        with pytest.raises(HTTPError):
            exa_client.query("trigger 500")

        def test_timeout_propagates(exa_client):
        with patch.object(ExaSearchClient, "_send_request", side_effect=Timeout):
        with pytest.raises(Timeout):
            exa_client.query("timeout test")

        def test_parse_response_skips_missing_id(exa_client):
        bad_payload = {
        "results": [
            {"title": "No ID here", "url": "http://bad.com", "score": 0.1},
            {"id": "good1", "title": "Good", "url": "http://good.com", "score": 0.9},
        ]
        }
        parsed = exa_client._parse_exa_response(bad_payload)
        assert len(parsed) == 1
        assert parsed[0].id == "good1"

        def test_parse_response_no_results_key_logs_and_returns_empty(exa_client):
        parsed = exa_client._parse_exa_response({"error": "some error"})
        assert parsed == []

        def test_default_num_results(monkeypatch, mock_exa_response):
        monkeypatch.setenv("EXA_API_KEY", "dummy_key")
        client = ExaSearchClient()
        with patch.object(ExaSearchClient, "_send_request", return_value=mock_exa_response):
        # caller omits num_results -> should pass default 10 into payload
        with patch.object(ExaSearchClient, "_send_request") as mock_send:
            mock_send.return_value = mock_exa_response
            client.query("default num")
            sent_payload = mock_send.call_args[1]["json_data"]  # kwargs
            assert sent_payload["num_results"] == 10

        def test_query_includes_date_filters(monkeypatch, mock_exa_response):
        monkeypatch.setenv("EXA_API_KEY", "dummy_key")
        client = ExaSearchClient()
        start = "2024-01-01"
        end = "2024-06-30"
        with patch.object(ExaSearchClient, "_send_request", return_value=mock_exa_response) as mock_send:
        client.query("battery tech", start_published_date=start, end_published_date=end)
        payload = mock_send.call_args[1]["json_data"]
        assert payload["startPublishedDate"] == start
        assert payload["endPublishedDate"] == end