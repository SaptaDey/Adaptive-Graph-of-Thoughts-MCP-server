...
def test_select_best_evidence_idempotent(evidence_list):
    ...
    assert once == twice
def test_gather_evidence_empty_openai_response(monkeypatch, sample_prompt, sample_thoughts):
    import adaptive_graph_of_thoughts.stage_4_evidence as s4
    monkeypatch.setattr(
        s4.openai.ChatCompletion,
        "create",
...
def test_gather_evidence_malformed_response(monkeypatch, sample_prompt, sample_thoughts):
    bad_contents = ["NoDelimiterHere"]
    import adaptive_graph_of_thoughts.stage_4_evidence as s4
    monkeypatch.setattr(
        s4.openai.ChatCompletion,
        "create",
...
def test_select_best_evidence_more_than_available(sample_evidence):
    result = select_best_evidence(sample_evidence, max_items=len(sample_evidence) + 5)
    assert len(result) == len(sample_evidence)
    assert result == sorted(sample_evidence, key=lambda e: e.score, reverse=True)

def test_select_best_evidence_negative_max(sample_evidence):
    with pytest.raises(ValueError):
        select_best_evidence(sample_evidence, max_items=-1)

def test_score_evidence_missing_score():
    class NoScore:
        def __init__(self, id):
            self.id = id
    with pytest.raises(AttributeError):
        score_evidence([NoScore("x")])

@given(st.lists(st.floats(allow_nan=False, allow_infinity=False), min_size=1))
def test_score_evidence_sorted_non_increasing(scores):
    evs = [Evidence(id=str(i), text="", score=s) for i, s in enumerate(scores)]
    sorted_evs = score_evidence(evs)
    sorted_scores = [e.score for e in sorted_evs]
