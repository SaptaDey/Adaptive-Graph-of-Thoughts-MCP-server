...
def get_neo4j_settings() -> Neo4jSettings:
    ...
    Initializes and caches the Neo4jSettings object on first call, loading connection parameters from environment variables or defaults.
    Returns:
        The Neo4jSettings instance containing connection configuration.
    """
...
def get_neo4j_driver() -> Driver:
    ...
    If the driver does not exist or is closed, creates a new driver using the current settings and verifies connectivity. Raises an exception if the connection cannot be established.
    """
    global _driver
...
async def execute_query(
    ...
    Runs the specified query as either a read or write transaction, using the configured or provided database. The query is executed in a separate thread to avoid blocking the event loop. Returns the list of records resulting from the query.
