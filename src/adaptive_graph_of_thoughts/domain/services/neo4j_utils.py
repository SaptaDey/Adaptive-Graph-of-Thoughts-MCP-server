...
def get_neo4j_settings() -> Neo4jSettings:
    """
    Retrieves a singleton instance of Neo4jSettings with connection configuration.
...
def get_neo4j_driver() -> Driver:
    ...
    If the driver does not exist or is closed, creates a new driver using connection settings and verifies connectivity. Raises an exception if the connection cannot be established.
    """
    global _driver
...
async def execute_query(
    ...
    Runs the specified query as either a read or write transaction, using the provided parameters and database. The transaction is executed in a separate thread to avoid blocking the event loop.
    
    Args:
        query: The Cypher query to execute.
        parameters: Optional dictionary of parameters to pass to the query.
        database: Optional database name; defaults to the configured default if not specified.
        tx_type: Transaction type, either 'read' or 'write'. Defaults to 'read'.
    
    Returns:
        A list of Neo4j Record objects resulting from the query.
