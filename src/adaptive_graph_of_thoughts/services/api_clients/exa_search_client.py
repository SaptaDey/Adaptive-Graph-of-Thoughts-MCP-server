...
class ExaSearchClient:
    def __init__(self, main_config: LegacyConfig):
        if (
            not main_config.exa_search
            or not main_config.exa_search.base_url
            or not main_config.exa_search.api_key
        ):
            logger.error(
                "Exa Search configuration (base_url, api_key) is missing in settings."
            )
    ...
    def _parse_exa_response(
        ...
        if "results" not in response_json:
            ...
            ):  # Exa might return an error message at the top level
        ...
        for res in response_json.get("results", []):
            ...
            try:
                ...
                logger.error(
                    f"Error creating ExaArticleResult for ID '{res.get('id')}': {e}. Data: {article_data}"
                )
        return articles
    ...
    async def search(
        ...
        if start_published_date:
            payload["startPublishedDate"] = (
                start_published_date  # API expects camelCase
            )
        ...
        try:
            ...
            logger.info(
                f"Successfully retrieved {len(articles)} results from Exa search for query: '{query}'."
            )
            return articles
            ...
            logger.error(
                f"Failed to decode JSON from Exa API search for query '{query}': {e}"
            )
            raise ExaSearchClientError(f"Exa API search JSON decode error: {e}") from e
        except Exception as e:
            logger.error(
                f"Unexpected error during Exa search for query '{query}': {e}",
                exc_info=True,
            )
    ...
    async def find_similar(
        ...
        try:
            ...
            articles = self._parse_exa_response(response_json)
            logger.info(
                f"Successfully retrieved {len(articles)} similar results from Exa for URL: '{url}'."
            )
            return articles
        except (APIHTTPError, APIRequestError) as e:
            ...
            if isinstance(e, APIHTTPError) and e.response_content:
                logger.error(f"Exa error response content: {e.response_content}")
            raise ExaSearchClientError(
                f"Exa API find_similar request failed: {e}"
            ) from e
        except ValueError as e:  # JSONDecodeError
            logger.error(
                f"Failed to decode JSON from Exa API find_similar for URL '{url}': {e}"
            )
            raise ExaSearchClientError(
                f"Exa API find_similar JSON decode error: {e}"
            ) from e
        except Exception as e:
            logger.error(
                f"Unexpected error during Exa find_similar for URL '{url}': {e}",
                exc_info=True,
            )
            raise ExaSearchClientError(
                f"Unexpected error in Exa find_similar: {e}"
            ) from e
...
async def main_exa_search_test():
    ...
    try:
        ...
        if (
            not main_app_config.exa_search
            or not main_app_config.exa_search.api_key
            or not main_app_config.exa_search.base_url
        ):
            logger.warning(
                "Exa Search config (api_key, base_url) not fully set in settings; test may use placeholders or fail."
            )
            if not main_app_config.exa_search:
                main_app_config.exa_search = ExaSearchConfig(
                    api_key="test_api_key_placeholder", base_url="https://api.exa.ai"
                )
        ...
        logger.warning(
            f"Could not load settings ({e}), using mock Exa Search config for testing."
        )
        main_app_config = Config(  # Changed settings = Settings()
            exa_search=ExaSearchConfig(
                api_key="YOUR_EXA_API_KEY_HERE", base_url="https://api.exa.ai"
            )
        )

    exa_config_section = (
        main_app_config.exa_search
    )  # Renamed exa_config to exa_config_section
    if (
        not exa_config_section
        or not exa_config_section.api_key
        or "YOUR_EXA_API_KEY_HERE" in exa_config_section.api_key
        or "placeholder" in exa_config_section.api_key
    ):
        logger.error(
            "Actual Exa API key is required to run this test. Please configure it in settings.yaml or environment variables (EXA_SEARCH__API_KEY). Skipping test."
        )
        return

    logger.info(
        f"Attempting Exa client test with API Key: ...{exa_config_section.api_key[-4:] if exa_config_section.api_key else 'N/A'}"
    )

    async with ExaSearchClient(
        main_config=main_app_config
    ) as client:  # Changed settings=settings to main_config=main_app_config
        try:
            # Test Search
            search_query = "latest advancements in battery technology"
            logger.info(f"--- Testing Exa Search for query: '{search_query}' ---")
            search_results = await client.search(
                search_query, num_results=2, type="neural", use_autoprompt=True
            )

            if search_results:
                logger.info(
                    f"Found {len(search_results)} Exa search results for '{search_query}':"
                )
                for i, res in enumerate(search_results):
                    ...
                    if res.highlights:
                        logger.info(
                            f"    Highlight (first): {res.highlights[0][:100]}..."
                        )
                ...
                if first_url_for_similar:
                    logger.info(
                        f"--- Testing Exa Find Similar for URL: {first_url_for_similar} ---"
                    )
                    similar_results = await client.find_similar(
                        first_url_for_similar, num_results=2
                    )
                    if similar_results:
                        logger.info(
                            f"Found {len(similar_results)} Exa similar results for '{first_url_for_similar}':"
                        )
                        ...
                        logger.info(
                            f"No similar results found by Exa for URL: {first_url_for_similar}"
                        )
                else:
                    logger.info(
                        "Skipping Find Similar test as first search result had no URL."
                    )
            else:
            ...
            logger.error(
                f"An unexpected error occurred during Exa Search client test: {e}",
                exc_info=True,
            )
