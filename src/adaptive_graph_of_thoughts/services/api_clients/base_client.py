...
class APIHTTPError(BaseAPIClientError):
    ...
    def __init__(
        self, status_code: int, response_content: Any, message: Optional[str] = None
    ):
...
class AsyncHTTPClient:
    ...
    def __init__(
        self,
        base_url: str,
        settings: Optional[LegacyConfig] = None,
        api_key: Optional[
            str
        ...
        self.api_key = (
            api_key  # Store if needed for common auth, though often header-specific
        )
    ...
    async def get(
        ...
        try:
            response = await self.client.get(
                url=f"/{endpoint}", params=params, headers=headers
            )
            response.raise_for_status()  # Raises HTTPStatusError for 4xx/5xx responses
            logger.debug(
                f"GET request to {self.base_url}/{endpoint} successful with status: {response.status_code}"
            )
            return response
        except httpx.HTTPStatusError as e:
            logger.error(
                f"HTTP error {e.response.status_code} for {e.request.url}: {e.response.text[:200]}"
            )
            raise APIHTTPError(
    ...
    async def post(
        ...
        try:
            response = await self.client.post(
                url=f"/{endpoint}", json=json_data, data=data, headers=headers
            )
            response.raise_for_status()
            logger.debug(
                f"POST request to {self.base_url}/{endpoint} successful with status: {response.status_code}"
            )
            return response
        except httpx.HTTPStatusError as e:
            logger.error(
...
if __name__ == "__main__":
    ...
    if (
        __name__ == "__main__"
    ):  # This nested check is a bit unusual, ensure it's what you intend
