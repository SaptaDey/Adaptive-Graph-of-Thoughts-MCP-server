...
def test_yaml_is_valid(yaml_path):
    ...
    assert isinstance(data, (dict, list)), f"Top-level YAML should be a mapping or list in {yaml_path}"
...
def test_json_is_valid(json_path):
    ...
    assert isinstance(data, (dict, list)), f"Top-level JSON should be a mapping or list in {json_path}"
