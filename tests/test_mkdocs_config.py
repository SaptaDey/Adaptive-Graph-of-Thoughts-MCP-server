...
class TestMkDocsConfigEdgeCases:
    ...
    def test_null_yaml_config(self, temp_dir):
        ...
        with open(null_config_path, "w") as f:
            f.write("---\n")
        with pytest.raises(
            ValueError, match="Configuration file contains no valid data"
        ):
    ...
    def test_invalid_navigation_structures(self):
        ...
        invalid_navs = [
            ...
            None,
            [{"": "empty-key.md"}],
            [{"key": "not-markdown-file.txt"}],
            [{"key": []}],
            [{"key": {"invalid": "nested.md"}}],
            [{"key1": "file1.md", "key2": "file2.md"}],
            [True],
            [{"key": 123}],
        ]
...
class TestMkDocsConfigAdvanced:
    ...
    def test_deeply_nested_navigation(self):
        ...
        deeply_nested_nav = [{"Level 1": [{"Level 2": [{"Level 3": "deep.md"}]}]}]
        assert validate_navigation(deeply_nested_nav) is True
        overly_nested_nav = [
            {"L1": [{"L2": [{"L3": [{"L4": [{"L5": "too_deep.md"}]}]}]}]}
        ]
    ...
    def test_performance_with_complex_config(self, temp_dir):
        ...
        complex_config = {
            ...
            "nav": [
                {
                    f"Section {i}": [
                        {
                            f"Subsection {i}-{j}": [
                                {f"Page {i}-{j}-{k}": f"page_{i}_{j}_{k}.md"}
                                for k in range(5)
                            ]
                        }
                        for j in range(10)
            ...
            "extra": {
                "social": [
                    {
                        "icon": f"fontawesome/brands/service{i}",
                        "link": f"https://service{i}.com/user",
                    }
                    for i in range(20)
                ],
...
class TestMkDocsConfigIntegration:
    ...
    def test_config_validation_with_file_system_checks(
        self, temp_dir, sample_mkdocs_config
    ):
        ...
        for filename in ["index.md", "getting-started.md", "about.md"]:
            with open(os.path.join(docs_dir, filename), "w") as f:
                f.write(
                    f"# {filename.replace('.md', '').replace('-', ' ').title()}\n\nContent here."
                )
        ...
        for filename in ["index.md", "auth.md"]:
            with open(os.path.join(api_dir, filename), "w") as f:
                f.write(
                    f"# API {filename.replace('.md', '').title()}\n\nAPI documentation."
                )
        config_path = os.path.join(temp_dir, "mkdocs.yml")
        with open(config_path, "w") as f:
    ...
    def test_config_backup_and_modification_scenarios(
        self, _temp_dir, mkdocs_config_file
    ):
        ...
        assert "This site has been modified" not in restored_config.get(
            "site_description", ""
        )
