...
def mock_gs_settings() -> Settings:
    ...
    Returns:
        A Settings instance containing a GoogleScholarConfig with a test base URL and API key.
    """
    return Settings(
...
async def gs_client_fixture(mock_gs_settings: Settings) -> GoogleScholarClient:
    """
    Asynchronous fixture that provides a GoogleScholarClient instance for testing.
    
    Yields:
        An initialized GoogleScholarClient within an async context manager.
    """
...
def test_gs_client_initialization_missing_config():
    """
    Verifies that GoogleScholarClientError is raised when the Google Scholar configuration is missing or incomplete.
    
    Checks that the client initialization fails if the API key or base URL is not provided in the settings.
    """
...
async def test_search_success(gs_client_fixture: GoogleScholarClient, httpx_mock: HTTPXMock):
    ...
    Verifies that all expected article fields are populated and that the API request includes the correct parameters.
    """
...
async def test_search_request_error(gs_client_fixture: GoogleScholarClient, httpx_mock: HTTPXMock):
    """
    Tests that a connection error during a search request raises a GoogleScholarClientError.
    
    Asserts that the underlying cause of the error is an APIRequestError.
    """
    client = gs_client_fixture
...
async def test_search_invalid_json_response(gs_client_fixture: GoogleScholarClient, httpx_mock: HTTPXMock):
    """
    Tests that a GoogleScholarClient raises a GoogleScholarClientError when the API returns invalid JSON.
    """
    client = gs_client_fixture
...
async def test_parsing_various_author_formats(gs_client_fixture: GoogleScholarClient, httpx_mock: HTTPXMock):
    """
    Tests that the GoogleScholarClient correctly parses article author information in various formats.
    
    Verifies that author fields are handled as comma-separated strings, single strings, or None when missing or empty.
    """
    client = gs_client_fixture
...
async def test_parsing_cited_by_count(gs_client_fixture: GoogleScholarClient, httpx_mock: HTTPXMock, caplog):
    """
    Tests that the client correctly parses the cited_by_count field from various response formats.
    
    Verifies that valid integer values are parsed, invalid or missing values result in None, and appropriate warnings are logged for unparsable data.
    """
    client = gs_client_fixture
