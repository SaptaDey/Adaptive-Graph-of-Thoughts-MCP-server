...
# ---------------------------------------------------------------------------
# Additional test cases (pytest + pytest_httpx)
# ---------------------------------------------------------------------------

async def test_search_retry_logic(exa_client_fixture: ExaSearchClient, httpx_mock: HTTPXMock):
    """
    When the first request returns 5xx the client should retry once and succeed.
    """
    client = exa_client_fixture

    # First mock response – transient server error
    httpx_mock.add_response(
        url=f"{client.config.base_url}/search",
        method="POST",
        status_code=502,
    ...
    articles = await client.search("retry query")

    # Two requests should have been issued and final result parsed correctly
    assert len(httpx_mock.get_requests()) == 2
    assert len(articles) == 2
    assert {a.id for a in articles} == {"exa_res_1", "exa_res_2"}


@pytest.mark.parametrize("bad_num_results", [-1, 0, "ten", 2.5])
async def test_search_invalid_num_results(exa_client_fixture: ExaSearchClient, bad_num_results):
    """
    Passing non-positive or non-integer `num_results` should raise ValueError.
    """
    client = exa_client_fixture
    with pytest.raises(ValueError, match="num_results must be a positive integer"):
        await client.search("invalid param", num_results=bad_num_results)


async def test_search_timeout_error(exa_client_fixture: ExaSearchClient, httpx_mock: HTTPXMock):
    """
    A timeout raised by httpx should be surfaced as ExaSearchClientError.
    """
    client = exa_client_fixture
    httpx_mock.add_exception(
    ...
    # Underlying cause should be APIRequestError (wrapped by the Exa client)
    assert isinstance(exc_info.value.__cause__, APIRequestError)


def test_async_context_manager_closes_client(mock_exa_settings: Settings):
    """
    Ensure that exiting the async context manager closes the underlying httpx.AsyncClient.
    """

    async def _open_and_return():
        async with ExaSearchClient(settings=mock_exa_settings) as c:
            return c

    client_instance = asyncio.run(_open_and_return())
    # `is_closed` is httpx.AsyncClient property indicating transport is closed.
    assert client_instance.http_client.client.is_closed