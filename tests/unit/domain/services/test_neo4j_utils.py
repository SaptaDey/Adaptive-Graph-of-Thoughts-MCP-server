...
def mock_runtime_settings():
    ...
    with patch(
        "adaptive_graph_of_thoughts.infrastructure.neo4j_utils.runtime_settings"
    ) as mock_settings:
...
def mock_neo4j_driver():
    ...
    with patch(
        "adaptive_graph_of_thoughts.infrastructure.neo4j_utils.GraphDatabase.driver"
    ) as mock_driver:
...
def reset_global_state():
    ...
    import adaptive_graph_of_thoughts.infrastructure.neo4j_utils as neo4j_utils
...
def mock_logger():
    ...
    with patch(
        "adaptive_graph_of_thoughts.infrastructure.neo4j_utils.logger"
    ) as mock_log:
        yield mock_log
...
class TestGetNeo4jDriver:
    ...
    def test_get_neo4j_driver_missing_neo4j_config(
        ...
        with patch(
            "adaptive_graph_of_thoughts.infrastructure.neo4j_utils.get_neo4j_settings"
...
class TestExecuteQuery:
    ...
    async def test_execute_query_driver_not_available(self, mock_logger):
        ...
        with patch(
            "adaptive_graph_of_thoughts.infrastructure.neo4j_utils.get_neo4j_driver"
        ) as mock_get_driver:
