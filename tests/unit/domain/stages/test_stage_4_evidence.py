...
@pytest.mark.asyncio
async def test_close_clients_tolerates_none_clients(evidence_stage_pubmed_only: EvidenceStage):
    stage = evidence_stage_pubmed_only
    # Explicitly clear non-existent clients to None
    stage.google_scholar_client = None
    stage.exa_client = None
    # PubMed client has AsyncMock.close; others are None
    # Should not raise
    await stage.close_clients()
    stage.pubmed_client.close.assert_called_once()

def _make_conf_vec(vals):
    return vals + [0.0] * (4 - len(vals))

def test_update_hypothesis_confidence_calculation(evidence_stage_all_clients: EvidenceStage):
    stage = evidence_stage_all_clients
    # Hypothetical old confidence vector & power
    old_vec = [0.2, 0.4, 0.6, 0.8]
    power = StatisticalPower.HIGH
    # Expected simple behaviour: HIGH should increase values modestly, clipping at 1.0
    expected = [min(v + 0.1, 1.0) for v in old_vec]
    result = stage._update_hypothesis_confidence(old_vec, power)
    assert all(math.isclose(a, b, rel_tol=1e-9) for a, b in zip(result, expected))

@pytest.mark.asyncio
async def test_execute_hypothesis_plan_handles_missing_optional_fields(evidence_stage_pubmed_only: EvidenceStage, sample_hypothesis_data):
    stage = evidence_stage_pubmed_only
    # Create article missing authors & doi
    article = PubMedArticle(pmid="pm1", title="Title", url="url", authors=[], publication_date=None, doi=None, abstract="")
    stage.pubmed_client.search_articles.return_value = [article]
    found = await stage._execute_hypothesis_plan(sample_hypothesis_data)
    assert found[0]['authors_list'] == []  # empty ok
