...
def valid_config(tmp_path):
    ...
    Returns:
        A tuple containing the file path to the YAML config and the configuration dictionary.
    """
    cfg = {
        "host": "localhost",
        "port": 8080,
...
def test_minimal_config(minimal_config):
    ...
    Asserts that required fields are preserved and optional fields default to expected values.
    """
...
def test_unreadable_file(tmp_path):
    ...
    Creates a valid config file, removes its read permissions, and asserts that validation fails due to file inaccessibility. Restores permissions after the test to allow cleanup.
    """
...
def test_boundary_values(tmp_path, port):
    ...
    Validates a configuration file where the port is set to a boundary value (e.g., 1 or 65535) and asserts that the validated configuration preserves the specified port.
    """
