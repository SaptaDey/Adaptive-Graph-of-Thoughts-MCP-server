...
def valid_config(tmp_path):
    ...
    Returns:
        A tuple containing the path to the created YAML file and the configuration dictionary.
    """
    cfg = {
        "host": "localhost",
        "port": 8080,
...
def test_minimal_config(minimal_config):
    ...
    Asserts that required fields are preserved and optional fields default to their expected values.
    """
...
def test_missing_keys(tmp_path, missing_key):
    ...
    The test removes either the "host" or "port" key from a valid configuration, writes it to a temporary file, and asserts that the error message references the missing key.
    """
...
def test_wrong_types(tmp_path, field, value):
    ...
    Args:
        tmp_path: Temporary directory for creating the config file.
        field: The configuration field to assign an invalid type.
...
def test_unreadable_file(tmp_path):
    ...
    Creates a config file with valid content, removes read permissions, and asserts that
    either a ConfigValidationError or PermissionError is raised when attempting validation.
    Restores file permissions after the test to allow cleanup.
    """
...
def test_boundary_values(tmp_path, port):
    """
    Tests that the configuration validator accepts boundary values for the port field.
    
    Verifies that a configuration file with the port set to its minimum or maximum allowed value is validated successfully and the port value is preserved in the result.
    """
