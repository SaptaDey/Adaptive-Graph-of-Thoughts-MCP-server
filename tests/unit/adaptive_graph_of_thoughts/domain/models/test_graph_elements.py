...
class TestGraphElementEdgeCases:
    ...
    @pytest.mark.parametrize(
        "weight",
        [0.0, sys.float_info.max, sys.float_info.epsilon, -sys.float_info.epsilon],
    )
...
class TestGraphElementThreadSafety:
    ...
    def test_concurrent_creation(self):
        ...
        def create_element(index):
            return GraphElement(
                node_id=uuid.uuid4(), label=f"thread_{index}", weight=float(index)
            )
        ...
        assert len({elem.node_id for elem in elements}) == 100  # All unique IDs
...
class TestGraphElementPropertyBased:
    ...
    def test_property_inequality_conditions(
        self, node_id1, node_id2, label1, label2, weight1, weight2
    ):
...
class TestGraphElementPerformance:
    ...
    def test_creation_performance(self):
        ...
        for i in range(10000):
            elements.append(
                GraphElement(
                    node_id=uuid.uuid4(), label=f"perf_test_{i}", weight=float(i)
                )
            )
    ...
    def test_memory_usage_estimation(self):
        ...
        for i in range(1000):
            elements.append(
                GraphElement(
                    node_id=uuid.uuid4(), label=f"memory_test_{i}", weight=float(i)
                )
            )
